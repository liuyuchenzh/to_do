 ...
 * Version: 0.3
 * Liscence: None
 * Note: 1. Intend to use template by directive. However, due to the limit of
 *       support from browsers over "file://" requests, decided to use one page
 *       only.
 *       2. Rmoved irrelavent functions that should be done in version 0.2.
 *       3. Add a simple trash can feature.
 *
**/

angular.module("toDoList", [])
.controller("listController", ["gevent", "convertDate", "setDisabled",
  "removeListOnStorage", "$scope",
  function (gevent, convertDate, setDisabled, removeListOnStorage, $scope) {
	var that = this;
	this.data = {
		lists: []
	};
  ...
  this.goToTrashCan = function() {
    ...
    var time = Date.now();
    var key = that.config.listKey + time;
  ...
  // end event handler /removeTrash/
    ...
    var key = that.config.listKey + Date.now();
    var val;
...
.controller("formController", ["gevent", "convertDate", "$scope",
  function (gevent, convertDate, $scope) {
	var that = this;
...
.controller("trashCanController", ["gevent", "removeListOnStorage", "$scope",
  function(gevent, removeListOnStorage, $scope) {
  var that = this;
  ...
  (function() {
    ...
    for (key in window.localStorage) {
      if (key.indexOf(that.config.listKey) === 0) {
        that.data.lists.push(JSON.parse(window.localStorage[key]));
      }
    }
...
.directive("ycDisabled", ["getNameSpace", function(getNameSpace) {
  var formRegExp = /form:\s*([\w_]+)\s*,?/,
      watchRegExp = /watch:\s*([\w_\.]+)\s*,?/;
  return {
    link: function(scope, elem, attrs, ctrl) {
      var match, formStr, watchStr, watch, opp;
      // first instinct is to use eval() to interpreate the attrs
      // however if one property value of ycDisabled is a property of an object
      // it will encounter an error bc that object needs to be defined in this
      // sepcific function scope
      // tried another way below

      // remove "{" and "}" from objStr
      var objStr = attrs["ycDisabled"].trim().slice(1, attrs["ycDisabled"].length - 1);
      // get form string
      match = formRegExp.exec(objStr);
      formStr = match[1];
      // get watch string, then use getNameSpace to get the watch value
      match = watchRegExp.exec(objStr);
      if (match) {
        watchStr = match[1];
